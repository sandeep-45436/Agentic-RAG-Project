    ...
    <AnimatedBackground>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {certifications.map((cert) => (
            <div
              key={cert.id}
              className="rounded-3xl bg-white/80 backdrop-blur-sm border border-slate-200 p-5 backdrop-blur flex flex-col justify-between hover:border-cyan-500/40 hover-lift shadow-xl space-y-4"
            >
              <div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white/80 backdrop-blur-sm border border-white/15 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleVerifyNew} className="space-y-3.5 text-xs">
              <div>
                <label className="text-slate-700 font-medium block mb-1">Certification Authority / Issuer</label>
                <Input
                  placeholder="e.g. Amazon Web Services, Google Cloud, Cisco"
                  value={issuer}
                  onChange={(e) => setIssuer(e.target.value)}
                  required
                  className="bg-slate-50 border-slate-200 text-white text-xs rounded-xl"
                />
              </div>

              <div>
                <label className="text-slate-700 font-medium block mb-1">Certification Official Title</label>
                <Input
              ...
              <div>
                <label className="text-slate-700 font-medium block mb-1">Credential ID / Verification Hash</label>
                <Input
              ...
              <div>
                <label className="text-slate-700 font-medium block mb-1">Issue Date</label>
                <Input
                  type="date"
                  value={issueDate}
