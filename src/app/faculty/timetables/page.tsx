    ...
    <AnimatedBackground>
      <div className="space-y-6">
          ...
          <Button
            ...
            className={`rounded-xl text-xs ${
              selectedDay === "ALL"
                ...
                : "border-slate-200 bg-white/80 text-slate-500 hover:text-slate-800 backdrop-blur-sm"
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            <div className="col-span-full py-16 text-center text-slate-500 text-xs animate-pulse">
            ...
            <div className="col-span-full py-16 text-center space-y-2">
              ...
              <p className="text-sm font-medium text-slate-700">No timetable entries scheduled</p>
              <p className="text-xs text-slate-500">Click &quot;Add Class Slot&quot; to assign a lecture or lab session.</p>
            </div>
          ) : (
            filteredTimetables.map((slot, idx) => (
              <Card
                key={slot.id}
                className={`bg-white/80 border-slate-200 hover:border-purple-300 transition-all backdrop-blur-sm group light-glass-card card-3d-inner anim-fade-up-${Math.min(idx + 1, 5)}`}
              >
                <CardHeader className={`pb-2 light-glass-card anim-fade-up-${Math.min(idx + 1, 5)}`}>
                  <div className="flex items-center justify-between">
                    <Badge className="bg-purple-50 text-purple-600 border-purple-200 font-mono text-xs">
                      {slot.courseCode}
                    </Badge>
                  ...
                  <CardTitle className="text-sm font-semibold text-slate-800 mt-1.5 truncate">
                    {slot.courseTitle}
                  </CardTitle>
                </CardHeader>
          ...
          <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
            <Card className="w-full max-w-lg bg-white border-slate-200 shadow-2xl">
              <CardHeader className="flex flex-row items-center justify-between pb-3">
                <div>
                  <CardTitle className="text-lg text-slate-800 font-semibold">Add Class / Lab Timetable Slot</CardTitle>
              ...
              <form onSubmit={handleAddSlot}>
                <CardContent className="space-y-3.5">
                  {formError && (
                    <div className="flex items-center gap-2 p-2.5 bg-rose-50 border border-rose-200 text-rose-600 text-xs rounded-xl">
                      <AlertCircle className="h-4 w-4 shrink-0" />
                      <span>{formError}</span>
                    </div>
                  )}
                  {formSuccess && (
                    <div className="flex items-center gap-2 p-2.5 bg-emerald-50 border border-emerald-200 text-emerald-600 text-xs rounded-xl">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-700">Course Code</Label>
                      <Input
                        value={courseCode}
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-700">Day of Week</Label>
                      <select
                        value={dayOfWeek}
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs text-slate-700">Course Title / Subject Name</Label>
                    <Input
                      value={courseTitle}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-700">Start Time</Label>
                      <Input
                        value={startTime}
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-700">End Time</Label>
                      <Input
                        value={endTime}
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs text-slate-700">Assigned Room / Hall</Label>
                    <Input
                      value={room}
            ...
            <Card className="w-full max-w-lg bg-white border-slate-200 shadow-2xl">
              ...
              <form onSubmit={handleSaveEdit}>
                <CardContent className="space-y-3.5">
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs text-slate-700">Course Title / Subject Name (Rename)</Label>
                    <Input
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-700">Academic Term</Label>
                      <Input
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    ...
                    <Button
                      ...
                      className="border-slate-200 text-slate-700 hover:text-slate-800 rounded-xl text-xs py-2.5"
                    >
                      Cancel
                    </Button>
                  </div>
        ...
        {showBulkModal && (
          <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
            <Card className="w-full max-w-lg bg-white border-slate-200 shadow-2xl">
              <CardHeader className="flex flex-row items-center justify-between pb-3">
                <div>
                  <CardTitle className="text-lg text-slate-800 font-semibold">Bulk Timetable Import</CardTitle>
                  <CardDescription className="text-xs text-slate-500">
              ...
              <CardContent className="space-y-3.5">
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs text-slate-700">Sample Template & CSV Data</Label>
                  <textarea
                    rows={7}
                    value={bulkData}
                    ...
                    className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs text-slate-800 font-mono focus:border-purple-500 focus:outline-none"
                  />
                </div>
