...
<div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((c) => (
          <Card key={c.id} className="border-slate-200 backdrop-blur space-y-3 light-glass-card card-3d-inner anim-fade-up-1">
            <CardHeader className="pb-2">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-800 text-base">{c.code}</span>
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0 bg-blue-500/10 text-blue-300 border-blue-500/30 font-mono">
                      {c.credits} Credits
                    </Badge>
                  </div>
                  <CardTitle className="text-sm font-semibold text-slate-700 mt-1">{c.title}</CardTitle>
                </div>
            ...
            <CardContent className="space-y-3 text-xs">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-slate-500 text-[11px] uppercase">Active Term Sections</p>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => {
                ...
                {c.sections?.length > 0 ? (
                  c.sections.map((sec: any) => (
                    <div
                      key={sec.id}
                      className="p-2.5 rounded-xl border border-slate-200 flex items-center justify-between text-xs hover:border-slate-200 transition-all"
                    >
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-slate-800">{sec.sectionCode}</span>
                          <span className="text-[10px] text-slate-500 font-mono">({sec.room})</span>
                        </div>
                        <p className="text-[11px] text-slate-500">{sec.scheduleText || "Mon/Wed 09:00 AM"}</p>
                      </div>
          ...
          <div className="border border-slate-200 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleCreateCourse} className="space-y-3 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Credits *</label>
                  <Input
                    required
                    type="number"
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Syllabus Overview & Description</label>
                <Input
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="border border-slate-200 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleCreateSection} className="space-y-3 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Section Code *</label>
                  <Input
                    required
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Assign Faculty Instructor</label>
                <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Schedule Slot</label>
                  <Input
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="border border-slate-200 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleUpdateSection} className="space-y-3 text-xs">
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Schedule Text</label>
                <Input
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Reason for Audit Log</label>
                <Input
