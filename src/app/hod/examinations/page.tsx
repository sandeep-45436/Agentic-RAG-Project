      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="bg-emerald-600 hover:bg-emerald-500 text-slate-800 text-xs font-semibold rounded-xl"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-slate-200 backdrop-blur light-glass-card card-3d-inner anim-fade-up-1">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500">Scheduled Examination</p>
        ...
        <Card className="border-slate-200 backdrop-blur light-glass-card card-3d-inner anim-fade-up-1">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500">Anti-Malpractice Algorithm</p>
        ...
        <Card className="border-slate-200 backdrop-blur light-glass-card card-3d-inner anim-fade-up-1">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500">Invigilator Allocation</p>
      ...
      <Card className="border-slate-200 backdrop-blur light-glass-card card-3d-inner anim-fade-up-1">
        <CardHeader className="pb-3 flex flex-row items-center justify-between">
          ...
          <Button
            ...
            className="bg-purple-600 hover:bg-purple-500 text-slate-800 text-xs rounded-xl"
          >
        ...
        <CardContent className="space-y-2 text-xs">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {invigilators.map((inv, idx) => (
              <div key={idx} className="p-3 rounded-xl border border-slate-200 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-800 text-xs">{inv.name}</span>
                  <Badge variant="outline" className="text-[9px] bg-purple-500/10 text-purple-300 border-purple-500/30">
                    {inv.status}
                  </Badge>
                </div>
                <p className="text-slate-500 text-[11px]">Venue: <strong className="text-slate-700">{inv.hall}</strong></p>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {seatings.map((seat) => (
              <div
                key={seat.id}
                className="p-3 rounded-xl border border-slate-200/80 space-y-1 text-xs hover:border-slate-200 transition-all"
              >
                <div className="flex items-center justify-between">
                ...
                <p className="font-bold text-slate-800 text-xs">{seat.name}</p>
                <p className="text-[11px] text-slate-500 font-mono">{seat.roll}</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="border border-slate-200 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleAppointInvigilator} className="space-y-3 text-xs">
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Select Faculty Instructor *</label>
                <select
                  required
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Exam Hall / Venue</label>
                <Input
                  value={appointHall}
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-200">
                ...
                <Button
                  ...
                  className="bg-purple-600 hover:bg-purple-500 text-slate-800 font-semibold text-xs rounded-xl"
                >
