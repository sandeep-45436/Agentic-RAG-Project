...
<div className="space-y-6 font-sans">
      ...
      <div className="relative max-w-md">
        ...
        <Input
          ...
          className="pl-10 border-slate-200 text-slate-800 placeholder:text-slate-600 text-xs rounded-xl"
        />
      </div>
            ...
            >
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-2">
                      <CardTitle className="text-base font-bold text-slate-800">{f.name}</CardTitle>
                      <Badge
                        variant="outline"
              ...
              <CardContent className="space-y-3 text-xs">
                {/* Details */}
                <div className="space-y-1 p-2.5 rounded-xl border border-slate-200/80 text-[11px]">
                  <p className="text-slate-700">
                    <strong className="text-slate-500">Specialization:</strong> {f.specialization}
                  </p>
                  <p className="text-slate-700">
                    <strong className="text-slate-500">Office Room:</strong> {f.officeRoom}
                  </p>
                  <p className="text-slate-700">
                    <strong className="text-slate-500">Email:</strong> {f.email}
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="text-slate-500 font-semibold uppercase text-[10px]">Workload Engine Load</span>
                    <span
                  ...
                  {f.sections?.length > 0 ? (
                    <div className="space-y-1">
                      {f.sections.map((sec: any) => (
                        <div key={sec.id} className="p-1.5 rounded-lg border border-slate-200 text-[11px] flex justify-between">
                          <span className="font-semibold text-slate-800">{sec.course?.code || "Course"} {sec.sectionCode}</span>
                          <span className="text-slate-500">{sec.scheduleText || "Mon/Wed"}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-slate-200/80">
                  <Button
                    ...
                    className="flex-1 border-slate-200 text-slate-700 hover:text-slate-800 text-xs rounded-xl"
                  >
            ...
            {invitationResult ? (
              <div className="space-y-4 text-xs">
                <div className="p-4 rounded-xl bg-emerald-950/30 border border-emerald-800/40 space-y-2 text-center">
                  ...
                  <h4 className="font-bold text-slate-800 text-sm">Faculty Member Appointed</h4>
                  <p className="text-slate-700">
                ...
                <div className="p-3 rounded-xl border border-slate-200 space-y-1.5">
                  <span className="text-[10px] uppercase font-bold text-slate-500">Secure Activation Token</span>
                  <div className="font-mono text-blue-300 p-2 rounded-lg border border-slate-200 select-all text-xs">
                ...
                <div className="flex justify-end">
                  <Button
                    ...
                    className="bg-blue-600 hover:bg-blue-500 text-slate-800 text-xs rounded-xl"
                  >
                    Done
                  </Button>
                </div>
              ...
              <form onSubmit={handleAppointFaculty} className="space-y-3 text-xs">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">University Email *</label>
                    <Input
                      required
                      type="email"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Academic Title</label>
                    <Input
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Departmental Designation</label>
                    <Input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Specialization</label>
                    <Input
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Office Room</label>
                    <Input
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="border border-slate-200 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleUpdateFaculty} className="space-y-3 text-xs">
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Lifecycle Status</label>
                <select
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="border border-slate-200 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Mandatory Relieving Reason</label>
                <Input
                  required
