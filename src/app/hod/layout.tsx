...
export default function HODLayout({ children }: { children: React.ReactNode }) {
  ...
  const toggleSidebar = () => {
    setIsCollapsed((prev) => {
      const next = !prev;
      if (typeof window !== "undefined") {
        localStorage.setItem("hod_sidebar_collapsed", String(next));
      }
      return next;
    });
  };
  ...
  const renderSidebarContent = (collapsed: boolean) => (
    <div className="flex flex-col h-full bg-white/95 backdrop-blur-xl border-r border-slate-200">
      ...
      {/* Active HOD Card */}
      {session && (
        <div className={`mx-2 mt-3 rounded-xl bg-blue-50/60 border border-blue-200/60 backdrop-blur transition-all ${
          collapsed ? "p-2 text-center" : "p-3 mx-3 mt-4"
        }`}>
          {collapsed ? (
            <div className="flex justify-center" title={`${session.name} (${session.hodCode})`}>
              <Avatar className="h-8 w-8 border border-blue-400/40 ring-2 ring-blue-300/30">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center ${collapsed ? "justify-center px-2 py-2" : "justify-between px-3 py-2"} rounded-xl text-xs font-medium transition-all ${
                isActive
      ...
      <div className="p-2 border-t border-slate-200/80 space-y-1.5 mt-auto">
        <Link
          ...
          className={`flex items-center ${collapsed ? "justify-center p-2" : "justify-between px-3 py-1.5"} text-xs font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100/80 rounded-lg transition-colors`}
        >
          <div className="flex items-center gap-2">
            <GraduationCap className="h-3.5 w-3.5 text-slate-500 shrink-0" />
            {!collapsed && <span>Faculty Portal</span>}
          </div>
        ...
        <Button
          ...
          title="HOD Sign Out"
          className={`w-full ${collapsed ? "justify-center p-2" : "justify-start px-3 py-1.5"} text-xs text-rose-600 hover:text-rose-700 hover:bg-rose-50 h-auto rounded-lg`}
        >
          ...
          <header className="h-16 shrink-0 border-b border-slate-200/80 bg-white/70 backdrop-blur-xl px-4 sm:px-6 flex items-center justify-between gap-3 shadow-xs">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <button
                ...
                className="hidden md:flex p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors"
                ...
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="relative inline-flex items-center">
                    ...
                    <select
                      ...
                      className="bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-800 pl-8 pr-7 py-1.5 rounded-lg focus:outline-none focus:border-blue-500 cursor-pointer appearance-none shadow-xs"
                    >
                  ...
                  <Badge variant="outline" className="hidden xl:inline-flex bg-blue-50 text-blue-700 border-blue-200 text-[10px]">
                    Dean Multi-Scope
                  </Badge>
                </div>
              ) : (
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              ...
              <Link
                href="/dashboard"
                className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-semibold border border-blue-200 shadow-xs transition-colors shrink-0"
